...
class TodoManagerWorksheet(TodoManager):
    ...
    def get_items(self) -> list[TodoItem]:
        return [TodoItem(x[1], getattr(TodoStatus, x[2]), string_to_datetime(x[3]), item_number=int(x[0]))
    ...
    def add_item(self, item: TodoItem):
        ...
        self.worksheet.insert_row([item_number, item.desc, item.status.name, datetime_to_string(item.date_added)],
                                  item_number + 1)
    ...
    def get_item(self, item_id: int) -> TodoItem:
        ...
        return TodoItem(row[1], getattr(TodoStatus, row[2]), string_to_datetime(row[3]), item_number=int(row[0]))
